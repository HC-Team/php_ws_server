...
if(isset($_GET['id']))
{
    ...
    $msg['msg']='test php push to '.$_GET['name'];
    $msg['id']=$_GET['id'];
    $result = $ws->send(json_encode($msg));
    header("location: /test.php");
    
}
else
{
    print '<h1>PHP push (php websocket client)</h1>';
    $msg['type']='chanel_list';
    $result = $ws->send(json_encode($msg));
    $bb=json_decode($result);
    foreach ($bb->data as $k=>$v)
    {
        print 'send to <a href="?id='.$k.'&name='.$v->name.'">'.$v->name.'</a><br>'; 
    }
}    
$ws->close();
